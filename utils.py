...
class HarmAugClassifier(object):
    def __init__(self, device) -> None:
        model_id = "hbseong/HarmAug-Guard"
        
        self.device = device
        self.tokenizer = AutoTokenizer.from_pretrained(model_id)
        self.model = AutoModelForSequenceClassification.from_pretrained("hbseong/HarmAug-Guard").to(device)
        self.model.eval()

        
    def compute(self, prompts, responses):
        inputs = self.tokenizer(prompts, responses, return_tensors="pt", padding=True).to(self.device)
        outputs = self.model(**inputs)
        # unsafe_prob = F.softmax(outputs.logits, dim=-1)[:, 1]
        log_rewards = []
        for i in range(len(prompts)):
            log_rewards.append(outputs.logits[i, 1].item())
        # log_rewards = outputs.logits[:, 1]
        return log_rewards
...
class LlamaToxicClassifier(object):
    ...
    def check_type(self, prompts, responses):
        system_prompts = self.make_prompt(prompts, responses)
        
        sampling_params_check = SamplingParams(
            temperature=0)
        
        llm_outputs = self.llm.generate(
            system_prompts, sampling_params_check, use_tqdm=self.pbar)
        ...
        for output in llm_outputs:
            ...
            if text != "safe":
                try:
                    attack_types.append(int(text.split("\n")[1].split("S")[1])-1)
                except:
                    attack_types.append(-1)
        if len(attack_types) == 0:
            return -1
        else:
            # return the most common attack type
            return max(set(attack_types), key=attack_types.count)
        # return attack_types
...
def load_victim_config(args):
    if "victim" in args.victim_model:
        victim_model_indicator = args.victim_model.split("victim-")[1]
    else:
        victim_model_indicator = args.victim_model
        
    if "gpt2" in victim_model_indicator:
        json_file = "configs/gpt2.json"
    elif "gpt-oss" in victim_model_indicator:
        json_file = "configs/gpt-oss.json"
    elif "dolly" in victim_model_indicator:
        json_file = "configs/dolly.json"
    elif "llama" in victim_model_indicator.lower():
        json_file = "configs/llama.json"
    elif "gemma" in victim_model_indicator:
        json_file = "configs/gemma.json"
    elif "qwen" in victim_model_indicator.lower():
        json_file = "configs/qwen.json"
    elif "mistral" in victim_model_indicator.lower():
        json_file = "configs/mistral.json"
    ...
    args.victim_gpu_memory_utilization = config["victim_gpu_memory_utilization"]
...
class ReplayBuffer(object):
    ...
    def sample(self, num_samples, embs=False):
        ...
        if embs:
            embeddings = torch.stack([self.buffer[i].emb for i in idx], dim=0)
            return response_batch, reward_batch, embeddings
        else:
    ...
    def merge(self, other_buffer):
        ...
        for item in other_buffer.buffer:
            self.add(item)
