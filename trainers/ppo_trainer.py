...
class PPOTrainer(object):
    def __init__(self, args) -> None:
        ...
        if "gpt" in args.victim_model or "dolly" in args.victim_model:
            ...
            self.toxicity_fn = LlamaToxicClassifier(version=version, gpu_memory_utilization=args.toxicity_gpu_memory_utilization)
        ...
        delimiter = ","
        self.csvlogger = CsvLogger(filename=f"{args.log_dir}/{args.exp_name}.csv",
                                   delimiter=delimiter,
                                   level=logging.INFO,
                                   add_level_nums=None,
    ...
    def simulate_experience(self, batch,  rbuffer, beta, max_len):
        results = self.get_online_samples(
            batch, max_len=max_len, temp=1.0)

        c_log_reward = results["c_log_reward"]
        true_c_log_reward = results["true_c_log_reward"]
        lm_log_reward = results["lm_log_reward"]
        log_reward = lm_log_reward + (true_c_log_reward / beta)
        ...
        for i in range(responses.size(0)):
            self.csvlogger.info(
                ['"'+decoded_responses[i]+'"', true_c_log_reward[i].item(), lm_log_reward[i].item()])
    ...
    def get_online_samples(self, batch, max_len, temp=1.0):
        ...
        lm_logreward, c_log_reward, decoded_responses, true_c_log_reward = self.get_logreward(
            batch, prompts_responses)

        results = {"lm_log_reward": lm_logreward,
                   "c_log_reward": c_log_reward,
                   "true_c_log_reward": true_c_log_reward,
                   "log_z": log_z,
                   "sum_logpf": sum_logpf,
    ...
    #     losses = delta**2
    #     return losses
    # def compute_reinforce_loss(self, log_z, sum_logpf, log_reward):
    #     losses = -sum_logpf * log_reward
    #     return losses
    def compute_ppo_loss(self, results, prompt_inputs, log_reward):
        ref_log_probs = results["sum_logpf"].detach()
        ...
        log_probs = torch.gather(
            log_prob, -1, labels.unsqueeze(2)).squeeze(2)
        log_probs = torch.where(pad_mask, 0.0, log_probs)
        log_probs = torch.sum(log_probs, 1)
        
        ratio = torch.exp(log_probs - ref_log_probs)
        clip_ratio = torch.clamp(ratio, 1 - self.args.clip_ratio, 1 + self.args.clip_ratio)
        
        advantages = log_reward - log_reward.mean()
        decoded_responses = results["decoded_responses"]
        curr_embs = self.sentence_encoder.encode(decoded_responses, convert_to_tensor=True)
        _, _, prev_embs = self.rbuffer.sample(len(decoded_responses), embs=True)
        # prev_embs = torch.stack([self.rbuffer.buffer[i].emb for i in range(len(self.rbuffer.buffer))])
        ...
        advantages = advantages + self.args.exploration_lamb * (1.0 - cos_sim.mean(1).detach())
        
        ppo_loss = -torch.min(ratio * advantages, clip_ratio * advantages)
        return ppo_loss
    ...
    def get_batch_metrics(self, batch, step, rbuffer,  max_len, beta,  train=True):
        ...
        results, batch = self.simulate_experience(
        ...
        # losses = self.compute_reinforce_loss(
        #     results["log_z"], results["sum_logpf"], tempered_log_reward
        # )
    ...
    def eval(self):
        ...
        for _ in tqdm(range(num_samples), desc="eval"):
            ...
            lm_log_reward, c_log_reward, decoded_responses, true_c_log_reward = self.get_logreward(
                batch, prompts_responses)
            ...
            all_true_c_log_reward.append(true_c_log_reward.cpu())
        ...
        all_true_c_log_reward = torch.cat(all_true_c_log_reward)
        ...
        metrics = {
            ...
            "true_c_log_reward/eval": all_true_c_log_reward.mean().item(),
