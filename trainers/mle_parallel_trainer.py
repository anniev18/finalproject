...
class MLEParallelTrainer(object):
    def __init__(self, args) -> None:
        ...
        few_shot_file_prefix = self.args.few_shot_file.split("seed")[0]
        
        few_shot_items = []
        for seed in range(self.args.iteration):
            few_shot_file = f"{few_shot_file_prefix}seed{seed}/dataset.json"
            with open(few_shot_file, "r") as f:
                few_shot_items.extend(json.load(f))
        
        with open(f"{few_shot_file_prefix}seed{self.args.seed}/dataset_parallel.json", "w") as f:
            json.dump(few_shot_items, f)
        # print(kyle)
        self.dataloader = get_dataloader("mle", self.tokenizer, 
                                        prompt_file=args.prompt_file, 
                                        sft_file=f"{few_shot_file_prefix}seed{self.args.seed}/dataset_parallel.json", 
